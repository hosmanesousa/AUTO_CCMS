...
public class DDR {
	...
	public List<File> fileOnFolder() {
		...
		for ( int i = 0; i < files.length; i++) {
			if ( !fileName[i].startsWith("ddr") ) {
				newRenameFile[i] = renameDDR(fileName[i]);
			}
		}
	...
	public File renameDDR(String fileName) {
					
		File file1 = new File(getPath() + fileName);
		File file2 = new File(getPath() + "ddr_" + fileName);
	...
	public void sortDDR(List<File> listName) {
			
		Collections.sort(listName);
	}
		
	public void copyDDR(String copyDestination) throws IOException{
	...
	public void moveDDR(String moveDDR) throws IOException{
		...
		String [] fileNames = new String[ficheiros.size()];
			for (int i = 0; i < ficheiros.size(); i++) {
				...
				File source = new File(getPath() + fileNames[i]);
				File destination = new File(moveDDR + fileNames[i]);
